...
##We can use both NIMBLE and JAGS to fit the latent state model.

##We'll use 22,000 iterations, after 2,000 burnin will leave 20,000 samples.
...
## Take a look at how quickly that ran, and the resulting effective sample size (ESS).
...
## Here is the *minimum efficiency* across all sampled parameters, which is the number of effective (independent) samples produced per second of runtime.
...
##Now we'll write the model in a different way.  I've written a custom distribution for use in the model specification, called `dOccupancy`.  You can see the definition of this in [definitions.R](https://github.com/danielturek/zib_glmm/blob/gh-pages/code/definitions.R).  As parameters it takes the occupancy probability `p_occ` and the observation probability `p_obs`, and it returns the marginalized probability density of the observation.

##You can see the new model specification below, without the latent `z` variables, and using this new custom `dOccupancy` distribution for the observations `y`.  This model can only be used with NIMBLE.
...
##We'll consider a large number of different MCMC algorithms.  This serves two purposes:
...
##Looking at the Posterior Summaries section at the bottom of the [comparisons page](http://danielturek.github.io/zib_glmm/html/dOccupancy.html), we see that all MCMC algorithms appear to have converged nicely to the same posterior distribution.  The only one with any noticeable differences is the posterior for `sigma_alpha`, but the differences are minor.  Also, for the purposes of your biological anlysis, the standard deviation of the site random effects is probably not the most critical model parameter.  We'll check convergence otherwise, but this gives us good confidence in the results.
...
##We'll accept this MCMC algorithm, log_shft_blk, as the "best" algorithm hereafter, although most all of these algorithms would be just fine to use.
...
##We'll use the mainstream Gelman & Rubin convergence diagnostic from the `coda` package.  We want to see all values near to 1.
...
## Let me know if you want any help on how to interpet or use any of these numbers.  As far as interpreting the actual `beta[i]` parameters, you'll have to just look back at the model specifications above to see what each `beta[i]` represents, but that's easy enough.
