...
load('../data/zib_data_v4_monthfixed.RData')
...
##```{r define-dOcc-model-month-fixed}
code <- nimbleCode({
    ...
    for(i in 1:12) {
        betamonth[i] ~ dnorm(0, 0.001)    ## new fixed effects for each month
    }
    for(i in 1:N) {
        logit(p_occ[i]) <- alpha[siteID[i]] + beta[4]*aet[i] + beta[5]*tmn[i] + beta[6]*tmx[i] + beta[7]*year[i] + betamonth[monthfixed[i]]
        logit(p_obs[i]) <- beta[1] + beta[2]*list_length[i] + beta[3]*year_list_length[i]
        y[i] ~ dOccupancy(p_occ[i], p_obs[i])
    }
})

constants <- list(N=N, nsite=nsite, aet=aet, tmn=tmn, tmx=tmx, year=year, monthfixed=monthfixed, list_length=list_length, year_list_length=year_list_length, siteID=siteID)

data <- list(y=y)

inits <- list(mu_alpha=0, sigma_alpha=1, alpha=rep(0,nsite), beta=rep(0,7), betamonth=rep(0,12))

modelInfo_monthfixed <- list(code=code, constants=constants, data=data, inits=inits, name='monthfixed')
##```
...
##```{r, eval = FALSE}
Rmodel <- nimbleModel(modelInfo_monthfixed$code,
                      modelInfo_monthfixed$constants,
                      modelInfo_monthfixed$data,
                      modelInfo_monthfixed$inits)
